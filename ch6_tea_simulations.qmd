...
summary (tea_data)
head(tea_data)
tea_data %>% group_by(condition) %>% summarise(mean = mean(tea_data$rating))

summary (tea_data_highn)
head(tea_data_highn)
tea_data_highn %>% group_by(condition) %>% summarise(mean = mean(tea_data$rating))
...
null_model <- tibble(sim = 1:1000) |> 
  mutate(data = map(sim, \(i) make_tea_data(n_total = 48, delta = 0))) |>  # simulate
  unnest(cols = data) # wrangle
...
```{r}
ggplot(data=null_model, aes(x=rating)) +
  geom_histogram(alpha=.8, bins=15)
```
...
-   each sim represents one simulation of n participants with the set delta
...
-   this histogram looks normal, centered around 0, because under the null hypothesis, there's no difference (delta = 0) between the two conditions milk first vs. tea first, any difference is due to chance, therefore under this hypothesis observed deltas are normally distributed around 0.
...
-   each dot represents the rating of an individual in either condition

-   confidence interval indicates that 95% percent of the time, the range is likely to capture the true population mean of tea/milk first average ratings

-   SE indicates the variance in the participants' ratings in each condition

To do: how does this change when you use the low-n experiment?

-   standard error will be larger, CI will be larger

```{r}
tea_ratings1 <- filter(tea_data, condition == "tea first")$rating
milk_ratings1 <- filter(tea_data, condition == "milk first")$rating
t.test(tea_ratings1, milk_ratings1, var.equal=TRUE)
...
for (this_sim in 1:1000) { ### for loop that iterates through each simulated experiment
  this_experiment = null_model %>% ### selects the null model dataset
    filter(sim==this_sim) ### filter out the results for the iterated simulation
  
  tea_ratings <- filter(this_experiment, condition == "tea first")$rating ### get tea first ratings 
  milk_ratings <- filter(this_experiment, condition == "milk first")$rating ### get milk first ratings
  
  output = t.test(tea_ratings, milk_ratings) ### run t test to see if there's a significant difference
  
  this_exp_output = tibble(pvalue = output$p.value)  ### compule p value for each simulation
...
-   when null is true, p-value distributed evenly
